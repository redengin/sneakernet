    ...
    : root(_root)
{
    ...
    for (const auto &entry : std::filesystem::recursive_directory_iterator(root))
    {
        ...
        if (filename.string().starts_with(INWORK_PREFIX))
        {
            ...
            if (ec)
            {
                ESP_LOGW(TAG, "failed to remove %s [%s]", entry.path().c_str(), ec.message().c_str());
            }
        }
...
Catalog::InWorkContent Catalog::addFile(const std::filesystem::path &filepath, const std::optional<std::filesystem::file_time_type> timestamp, const size_t size)
{
