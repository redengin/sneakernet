...
public:
    ...
    /// HTTPS only used for capport api
    WebServer(const size_t max_sockets);

    /// registers handler for HTTP
...
private:
    ...
    static constexpr size_t MAX_HTTPS_URI_HANDLERS = 1; /* only used for capport api */

    httpd_handle_t httpHandle;
    static constexpr size_t MAX_HTTP_URI_HANDLERS =
        9   /* webapp handlers */
        + 4 /* catalog handlers */
        + 1 /* firmware handlers */;
};
