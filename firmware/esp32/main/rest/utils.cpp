...
bool rest::receiveOctetStream(httpd_req_t *const request, const int &fid)
{
  // create a chunk buffer
  std::unique_ptr<char[]> buffer = std::make_unique<char[]>(rest::CHUNK_SIZE);
  if (!buffer)
  ...
  while (true)
  {
    const auto received = httpd_req_recv(request, buffer.get(), rest::CHUNK_SIZE);

    if (received > 0)
    {
      const auto written = write(fid, buffer.get(), received);
      if (written < 0)
      {
        ESP_LOGE(TAG, "write failure [%d]", written);
    ...
    {
      ESP_LOGE(TAG, "read failure [%d]", received);
      httpd_resp_set_status(request, HTTPD_500);
      return false;
    }
    else if (received == 0)
